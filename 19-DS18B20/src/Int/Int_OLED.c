...
//设置页、列指针
void Int_OLED_SetPointer(u8 column, u8 page)
{
    u8 pointer[3];
    //设置页指针
    pointer[0] = 0xB0 | page;
    //设置列指针
    pointer[1] = 0x00 | (column & 0x0F);
    pointer[2] = 0x10 | (column >> 4);
    Int_OLED_WriteCmd(pointer, 3);
}
...
void Int_OLED_ShowStr(u8 x, u8 y, u8* str)
{
    ...
    addring_mode[0] = 0x20;
    addring_mode[1] = 0x02;
    Int_OLED_WriteCmd(addring_mode, 2);
    //设置页,列指针
...
void Int_OLED_ShowNum(u8 x, u8 y, long int num)
{
    ...
    while (tmp > 0)
    {
        str[i++] = num / tmp + '0';//拿到此数字的ASCII码值，才能和我们的showstr相对应
        num %= tmp;
        tmp /= 10;
    }
